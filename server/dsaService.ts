...
export async function recordDsaAttempt(pool: Pool, legacyUserId: string, problemLegacyId: string, status: 'SOLVED' | 'ATTEMPTED', timeTakenMinutes: number, notes: string, codeSnippet?: string) {
  ...
  try {
    ...
    const submission = await client.query(
      ...
      [id, problem.rows[0].id, calculateDayInfo().currentDate, status, Math.max(0, Number(timeTakenMinutes) || 0), notes || '', codeSnippet || null, nowIso]
    );
    let pointsAwarded = 0;
    if (status === 'SOLVED' && !wasSolved) {
      pointsAwarded = Number(problem.rows[0].points);
      await client.query(
        `INSERT INTO points_ledger (participant_id, challenge_id, challenge_day_id, amount, event_type, reason, metadata, created_at)
         VALUES ($1, $2, $3, $4, 'DSA_COMPLETED', $5, $6::jsonb, $7)`,
        [id, problem.rows[0].challenge_id, problem.rows[0].challenge_day_id, pointsAwarded, `Solved DSA Problem: ${problem.rows[0].title} (+${pointsAwarded} pts)`, JSON.stringify({ sourceProblemId: problemLegacyId }), nowIso]
      );
    }
